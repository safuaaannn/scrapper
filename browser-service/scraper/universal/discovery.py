...
CHECK_NEW_CONTENT_JS = """() => {
    ...
    const containers = document.querySelectorAll(
        ...
        '[class*="overlay"], [class*="panel"], [class*="sidebar"], [class*="slide-in"], ' +
        'dialog[open], size-guide-drawer, [class*="sizeguide"], [class*="size-guide"]'
    );
    for (const c of containers) {
        if (c.offsetParent !== null || getComputedStyle(c).display !== 'none') {
            // Found a visible overlay — check if it has tables, measurement content, or size chart images
            const tables = c.querySelectorAll('table');
            const text = c.textContent.toLowerCase();
            if (tables.length > 0 || text.includes('size') || text.includes('measurement')) {
                return 'modal';
            }
            // Check for size chart images (e.g., Westside: <img id="sizeGuideImage">)
            for (const img of c.querySelectorAll('img')) {
                const attrs = (img.id || '') + ' ' + (img.className || '') + ' ' +
                    (img.alt || '') + ' ' + (img.src || '');
                const dataCm = img.getAttribute('data-cm') || '';
                if (/size|sizing|measurement|chart|guide/i.test(attrs) || dataCm) {
                    return 'modal';
                }
            }
        }
...
async def discover_size_chart(page) -> str:
    ...
    BROAD_CONTENT_CHECK_JS = """() => {
        // Check for modals/drawers with tables or size chart images
        const containers = document.querySelectorAll(
            '[role="dialog"], [class*="modal"], [class*="drawer"], [class*="popup"], ' +
        ...
        for (const c of containers) {
            if (c.offsetParent !== null || getComputedStyle(c).display !== 'none') {
                ...
                // Check for size chart images in the modal
                for (const img of c.querySelectorAll('img')) {
                    const attrs = (img.id || '') + ' ' + (img.className || '') + ' ' +
                        (img.alt || '') + ' ' + (img.src || '');
                    const dataCm = img.getAttribute('data-cm') || '';
